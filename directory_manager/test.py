...
class WordsService:
    ...
    def removeWord(self):
        ...
        for word in wordList:
            if word.getEng() == del_word:  # word의 영단어 요소와 삭제할 단어 문자열이 일치하면 조건문 실행
                wordList.remove(word)    # 리스트의 remove() 메소드를 통해 해당 영단어 요소 삭제
    ...
    def editWord(self):
        ...
        for word in wordList:
            if word.getEng() == edit_word:  # word의 영어 단어를 가져와서 edit_word와 비교하고 만약 일치하는 단어를 찾으면 아래 코드실행
                new_meaning = input('업데이트 할 뜻을 입력하세요: ')  # 수정할 단어를 변수화하고 입력함수를 통해 초기화
                word.setKor(new_meaning)  # 수정단어 대입
    ...
    def loadDataFromFile(self):
        if os.path.exists(filename):  # 해당 파일이 존재하는지 확인
...
class Menu:
    ...
    def run(self):
        while True:
            try:
                menu = int(input('1. 등록 2. 출력 3. 저장 4. 수정하기 5. 삭제하기 6. 종료'))
                if menu == 1:
